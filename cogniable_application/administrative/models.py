...
import datetime
# from school.emails import school_activation_mail
from django.conf import settings
...
from administrative import signals
...
# from webapp.emails import forgot_email, school_activation_mail
...
class EmailConfirmation(models.Model):
    ...
    def send(self, template_context=None):
        ...
        subject = render_to_string('email/email_confirmation_subject.txt', template_context)
        subject = ''.join(subject.splitlines()).strip()  # remove unnecessary line breaks
        body = render_to_string('email/email_confirmation_message.html', template_context) 
        message = EmailMessage(subject, body, settings.DEFAULT_FROM_EMAIL, [self.email, ])
        message.content_subtype = 'html'
        message.send()
        # response = school_activation_mail('Activate your Account', [user_obj.email], 'User', user_obj.username, password)
    ...
    def confirm(self, ignore_expiration=False, save_to_content_object=False):

...
class Goals extends React.Component {
  ...
  render() {
    const {isLoaded, longTermGoalsList, selectedLongTermObject, selectedType, selectedShortTermObject, learnerId} = this.state;
    const { goals: {Loading, LongTermGoalsList, SelectedType, LongTermObject, ShortTermObject}} = this.props;
    if (Loading) {
                    ...
                    <section className="card">
                        <div className="card-header mb-2">
                            <div className="utils__title">
                                <strong>Goals 
                                    <span><Button style={{float:"right"}} onClick={this.showCreatGoal}><Icon type="plus" /> LTG</Button></span>
                                    <span style={{float:"right", marginRight:"5px", marginTop:"-4px", textDecoration:"none"}}>
                                        <Form.Item label="">
                                            <Select style={{ width: '120px', textDecoration:'none' }} defaultValue='active'>
                        ...
                        <div className="card-body">
                            ...
                            >
                                {LongTermGoalsList.map((item, index) => 
                                    ...
                                    <TreeNode icon={<Icon type="hdd" />} title={item.node.goalName} key={item.node.id} goal="LTG">
                                        {item.node.shorttermgoalSet.edges.length > 0 ? 
                                        
                                            item.node.shorttermgoalSet.edges.map((shortTermItem, lIndex) => 
                                                <TreeNode icon={<Icon type="file-protect" />} title={shortTermItem.node.goalName} key={shortTermItem.node.id} goal="STG">
                                                    {shortTermItem.node.targetAllocateSet.edges.length > 0 ?
                                                    
                                                        shortTermItem.node.targetAllocateSet.edges.map((targetItem, targetIndex) =>
                                                            <TreeNode title={targetItem.node.targetAllcatedDetails.targetName} key={targetItem.node.id} goal="Target" />

...
@connect(({ goals }) => ({ goals }))
class ManuallyTarget extends React.Component {
  // constructor(props) {
  ...
  //   const { form, goal, targets, learnerId } = this.props;
    ...
    client
      .query({
        query: gql`{
            target(
                domain:"${domainid}",
                targetArea_Area_Icontains:"other / manual", 
                targetMain_TargetName_Icontains:"other/ manual"
            )
            {
                edges {
                    node {
...
const ManualTargetForm = Form.create()(ManuallyTarget)
export default ManualTargetForm

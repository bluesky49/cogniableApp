...
class AllocateTarget extends React.Component {
  ...
  handleTargetSubmit = (e) => {
    ...
    const { form, targets, goals:{LearnerId, ShortTermObject} } = this.props;
  ...
  render() {
    ...
    console.log(targets)
    const { suggestedSd, fetchingsd, fetchingstep, suggestedStep, targetLength, targetInitialValue } = this.state;
    const itemStyle = {marginBottom:"5px"}
   
    return (
      <>
        <h6>Target {targetInitialValue}/{targetLength} : {targets[targetInitialValue - 1].node.targetMain.targetName}</h6>
        <br />

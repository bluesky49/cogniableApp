    ...
    return (
      <Form {...layout} onSubmit={(e) => this.SubmitForm(e, this)} name="control-ref" style={{marginLeft:"10%"}}>
        <Form.Item label="Condition">
          {form.getFieldDecorator('condition', {rules: [{ required: true, message: 'Please enter Condition!' }],})(
            <Input name="condition" style={{width:250}} />
          )}
        </Form.Item>

        <Form.Item label="Date">
          {form.getFieldDecorator('date', {rules: [{ required: true, message: 'Please pick a date!' }],})(
            <DatePicker onChange={this.setDate} name="date" style={{ width: 250 }} />
          )}
        </Form.Item>

        <Form.Item label="Severity">
          {form.getFieldDecorator('severity', {rules: [{ required: true, message: 'Please Select severity!' }],})(
            <Select
              showSearch
              style={{ width: 250 }}
              placeholder="Select"
        ...
        <Form.Item label="Drug">
          {form.getFieldDecorator('medicineDetails', {rules: [{ required: true, message: 'Please enter Medicine!' }],})(
            <Input name="medicineDetails" style={{width:250}} />
          )}
        </Form.Item>

        <Form.Item label="Dosage">
          {form.getFieldDecorator('dosage', {rules: [{ required: true, message: 'Please enter Dosage!' }],})(
            <Input name="dosage" suffix="mg" style={{width:"90px"}} />
          )}
          {form.getFieldDecorator('howOftenTaken', {rules: [{ required: true, message: 'Please enter Times!' }],})(
            <Input name="howOftenTaken" suffix="times a day" style={{width:"140px",marginLeft:"20px"}} />
          )}
        </Form.Item>

...
class TargetRecordingBlock extends React.Component {
  ...
  addInCorrectTrail = val => {
    ...
    const {
      dispatch,
      datarecording: { count, correct, incorrect },
    } = this.props
  ...
  changeStimulus = () => {
    const {
      dispatch,
      datarecording: { TargetInitialValue, TargetList, StimulusActiveIndex, StimulusActiveKey },
    } = this.props
    ...
    dispatch({
      ...
      payload: {
        ...
        Incorrect: [],
        StimulusActiveKey:
          TargetList[TargetInitialValue - 1].node.sd.edges[StimulusActiveIndex].node.id,
  ...
  previousStimulus = () => {
    ...
    dispatch({
      ...
      payload: {
        ...
        Incorrect: [],
        StimulusActiveKey:
          TargetList[TargetInitialValue - 1].node.sd.edges[StimulusActiveIndex - 1].node.id,
        StimulusActiveIndex: StimulusActiveIndex - 1,
      },
    })
  ...
  changeTarget = () => {
    const {
      dispatch,
      datarecording: { TargetInitialValue, TargetList, TargetActiveKey, RecordingType },
    } = this.props
    ...
    if (RecordingType === 'Behavior') {
      ...
      document.getElementsByClassName('targetElements')[TargetInitialValue].style.backgroundColor =
        '#bae7ff'
  ...
  addCount(This, val) {
    const {
      dispatch,
      datarecording: { TargetActiveKey, correct, incorrect, count },
    } = this.props
  ...
  addStimulusCount(This, val) {
    const {
      dispatch,
      datarecording: { StimulusActiveKey, TargetActiveKey, correct, incorrect, count },
    } = this.props
    if (count > correct + incorrect) {
      this.addCorrectTrail(val)
      dispatch({
  ...
  addStepCount(This, val) {
    const {
      dispatch,
      datarecording: { StepActiveKey, TargetActiveKey, correct, incorrect, count },
    } = this.props
    if (count > correct + incorrect) {
      this.addCorrectTrail(val)
      dispatch({
  ...
  render() {
    ...
    for (
      i = 0;
      i < TargetList[TargetInitialValue - 1].node.targetAllcatedDetails.DailyTrials;
      i++
    ) {
        ...
        >
          ...
          >
            <h5 style={{ textAlign: 'center', minHeight: '70px' }}>
              <a>{TargetList[TargetInitialValue - 1].node.targetAllcatedDetails.targetName}</a>
              <br />
            ...
            <p>
              ...
              <span style={{ float: 'right' }}>
                {TargetList[TargetInitialValue - 1].node.targetStatus.statusName}
              </span>
            </p>
                ...
                {TargetList[TargetInitialValue - 1].node.sd.edges.length > 0 ? (
                  <>
                    <p style={{ textAlign: 'center' }}>
                      Stimulus {StimulusActiveIndex}/
                      {TargetList[TargetInitialValue - 1].node.sd.edges.length}
                    </p>
                    ...
                    <Row>
                      <Col xs={4} style={{ height: '200px', paddingTop: '15%', paddingLeft: '7%' }}>
                        {StimulusActiveIndex > 1 ? (
                          <>
                            <Button type="link" onClick={this.previousStimulus}>
                      ...
                      <Col xs={16} style={{ borderRadius: '8px', backgroundColor: '#F5F5F5' }}>
                        <div id="guardadoC">
                          <p style={{ textAlign: 'center', marginTop: '15px' }}>
                            {
                              TargetList[TargetInitialValue - 1].node.sd.edges[
                                StimulusActiveIndex - 1
                              ].node.sd
                            }
                          </p>
                          <p style={{ textAlign: 'center', marginTop: '25px' }}>
                            <span>
                              {count === 1 ? (
                                ...
                                <Button onClick={this.moveToPrevTrail} type="link">
                                  <Icon type="arrow-left" />
                                </Button>
                              )}
                            </span>
                            Trial {count}/
                            {
                              TargetList[TargetInitialValue - 1].node.targetAllcatedDetails
                                .DailyTrials
                            }
                            <span>
                              {count ===
                              TargetList[TargetInitialValue - 1].node.targetAllcatedDetails
                                ...
                                <Button onClick={this.moveToNextTrail} type="link">
                                  <Icon type="arrow-right" />
                                </Button>
                              )}
                            </span>
                            ...
                            TargetList[TargetInitialValue - 1].node.targetAllcatedDetails
                              ...
                              <>
                                <Button
                                  ...
                                  onClick={() => this.addStimulusCount(this, count, correct)}
                                >
                          ...
                          <p style={promptOptionsDiv}>
                            {promptCodesList.map(item => (
                              <>
                                <Button
                                  ...
                                  onClick={() => this.promptStimulusCount(this, count, item)}
                                >
                                  {item.promptName}
                                </Button>
                                <br />
                            ...
                            <Button
                              style={promptCodeButtonStyle}
                              onClick={() => this.removeStimulusCount(this, count)}
                            >
                              No Response
                            </Button>
                          </p>
                      ...
                      <Col
                        xs={4}
                        style={{ height: '200px', paddingTop: '15%', paddingRight: '10%' }}
                      >
                        {TargetList[TargetInitialValue - 1].node.sd.edges.length >
                        StimulusActiveIndex ? (
                          <>
                            <Button onClick={this.changeStimulus} type="link">
                              <Icon type="arrow-right" />
                            </Button>
                          </>
                        ) : (
                  ...
                  <Collapse
                    ...
                    defaultActiveKey={
                      TargetList[TargetInitialValue - 1].node.steps.edges[0].node.id
                    }
                  >
                          ...
                          TargetList[TargetInitialValue - 1].node.targetAllcatedDetails
                            ...
                            <>
                              <Button
                                ...
                                disabled
                                style={{ padding: '20px auto', width: '300px', height: '50px' }}
                                onClick={() => this.addStepCount(this, count, correct)}
                              >
                                <CheckOutlined /> Correct ({correct})
                              </Button>
                              <Button
                        ...
                        <p style={promptOptionsDiv}>
                          {promptCodesList.map(item => (
                            <>
                              <Button
                                style={promptCodeButtonStyle}
                                onClick={() => this.promptStepCount(this, count, item)}
                              >
                                {item.promptName}
                              </Button>
                              <br />
                          ...
                          <Button
                            style={promptCodeButtonStyle}
                            onClick={() => this.removeStepCount(this, count)}
                          >
                            No Response
                          </Button>
                        </p>
                  ...
                  <>
                    <p style={{ textAlign: 'center', marginTop: '40px' }}>
                      ...
                      Trial {count}/
                      {TargetList[TargetInitialValue - 1].node.targetAllcatedDetails.DailyTrials}
                      <span>
                        {count ===
                        TargetList[TargetInitialValue - 1].node.targetAllcatedDetails
                          .DailyTrials ? (
                    ...
                    <p style={promptOptionsDiv}>
                      {promptCodesList.map(item => (
                        <>
                          <Button
                            style={promptCodeButtonStyle}
                            onClick={() => this.promptCount(this, count, item)}
                          >
                            {item.promptName}
                          </Button>
                          <br />
            ...
            <p style={{ marginTop: '20px' }}>
              Target {TargetInitialValue}/{TargetListLength}
              <span style={{ float: 'right' }}>
                {TargetList.length > TargetInitialValue ? (
                  <>
                    <Button onClick={this.changeTarget} type="primary">
                      Next
                    </Button>
                    &nbsp;

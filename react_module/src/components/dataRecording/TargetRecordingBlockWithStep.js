...
import {Card, Button, Drawer, Tooltip, Collapse } from 'antd';
import { LeftOutlined,RightOutlined,CheckOutlined,CloseOutlined } from '@ant-design/icons';

const {Panel} = Collapse;

class TargetRecordingBlockWithStep extends React.Component {
  ...
  render() {
    const {count,correct, percentage, incorrect, showPromptOptions, totalTrails, collapseCount, totalSteps, stepCount, stepTrails } = this.state;
            ...
            <Card style={{ width: '100%', backgroundColor:'#fbfbfb'}}>
              <Card style={{ width: '96%', backgroundColor: 'white', margin:'10px auto', minHeight:"540px"}}>
                ...
                <p style={{textAlign:'center'}}>
                  {spanList.map((item) => <span><span id={item} className="spanListItems" style={{height:"6px", width:"70px", border:"1px solid gray", backgroundColor:"rgba(192,192,192,0.3)", paddingLeft:"35px", borderRadius:"8px"}}>&nbsp;</span>&nbsp;&nbsp;</span>)}
                </p>
                <br />
                <Collapse accordion activeKey={collapseCount}>
                  <Panel header="Step 1 : Instruction 1" key="1">
                    
                    <p style={{textAlign:'center'}}>Trail {count}/{totalTrails}</p>
                    <p style={correctIncorrectDiv}>
                      <Button id="correctResponseButton" style={{padding:'20px auto', width:"300px", height:"50px"}} onClick={()=> this.addCount(this, count)}><CheckOutlined /> Correct ({correct})</Button>
...
export default TargetRecordingBlockWithStep
